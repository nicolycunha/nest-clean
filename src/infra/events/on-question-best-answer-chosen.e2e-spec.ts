...
describe('On question best answer chosen (E2E)', () => {
  ...
  it('should send a notification when question best answer is chosen', async () => {
    ...
    const accessTokenQuestionUser = jwt.sign({
      sub: questionAuthor.id.toString()
    })
    const accessTokenAnswerUser = jwt.sign({ sub: answerAuthor.id.toString() })
    ...
    await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${accessTokenQuestionUser}`)
      .send()
